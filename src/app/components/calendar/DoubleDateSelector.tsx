...
export const DoubleDateSelector: React.FC<IProps> = React.memo(
    ({ initialDate, onChange, className }) => {
        ...
        return (
            <>
                <DatePicker
                    ...
                    inputClassName={`${className}__create-event-form__input`}
                    wrapperClassName={`${className}__create-event-form__input-day`}
                    dateFormat='MMMM d, yyyy'
                />
                <div
                    className={`${className}__create-event-form__hour-selector`}
                >
                    <HourPicker
                        selected={dateBegin}
                        onChange={onDateBeginChange}
                        inputClassName={`${className}__create-event-form__input__hour`}
                        timeCaption='From'
                        wrapperClassName={`${className}__create-event-form__input-hour`}
                    />
